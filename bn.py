...
class BayesianNetwork(object):
    ...
    def construct(self):
        ...
        for var in self.prior_probabilities:
            self.graph[var].set_prob_table(
                self.construct_prior_prob_table(var))
        ...
        for var in self.dependencies:
            ...
            self.graph[var].set_prob_table(
                self.construct_conditional_prob_table(var))
    ...
    def infer(self):
        for i in self.queries:
            ...
            varying_variables = [
                y for y in self.variables if y not in fixed_variables]
            # print(given)
            part1 = self.calculate_prob(
                given, tofind, fixed_variables, varying_variables)
            # print(given)

            fixed_variables = [x for x in given]
            varying_variables = list(set(
                [y for y in self.variables if y not in fixed_variables] + [x for x in tofind]))
            part2 = self.calculate_prob(
                given, tofind, fixed_variables, varying_variables)
            answer = part1 / part2
            self.answer.append(
                {"index": index, "evidence": given, "toFind": tofind, "answser": answer})
            print({"index": index, "evidence": given,
                   "toFind": tofind, "answser": answer},"\n\n")
        # print(self.answer)
        return self.answer
    ...
    def calculate_prob(self, given, tofind, fixed_variables, varying_variables):
        ...
        all_possible_truth_values = self.perm(
            table_of_variables, len(varying_variables))
        ...
        for iteration in range(2 ** len(varying_variables)):
            ...
            # setting the varying variables to their new values
            ...
            # prob here contains all the probabilities of our individual variables
    ...
    def printGraph(self):
        for var in self.graph:
            node = self.graph[var]
            print("Node: {} Parent: {} Children: {}".format(
                node.name, node.parents, node.children))
    
    def jointProbDist(self):
        table={"headers":[x for x in self.variables],"data":[]}
        table['headers'].append("Probability")
        truth_table_values=list(product([True,False],repeat=len(self.variables)))
        for iteration in truth_table_values:
            row=list(iteration)
            table_of_variables={}
            for index, var in enumerate([x for x in self.variables]):
                table_of_variables[var] = str(iteration[index])
            prob = self.val([self.get_every_prob(table_of_variables)])
            row.append(prob)
            table['data'].append(row)
        print(tabulate(table['data'],headers=table['headers']),'\n')
